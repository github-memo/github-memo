...
public int insertStuduser (String stud_id, String stud_passwd,
...
boolean flag=false;
String sql=
"select * from studuser where stud_id = ?";
try {
conn = DriverManager.getConnection(url, user, pass);
ps = conn.prepareStatement(sql);
...
if (rs.next()) {
if (stud_id.equals(rs.getString("stud_id").trim()) &&
stud_passwd.equals(rs.getString("stud_passwd").trim())) {
...
list=makelist(rs);
return list;
} finally { if (ps!=null) { ps.close(); } if (conn!=null) { conn.close(); } if (rs!=null) { rs.close(); } }
}

public List<StuduserDTO> findHobbyStuduser(String stud_search) throws SQLException {
...
public List<StuduserDTO> findNameStuduser(String stud_search, String search_select) throws SQLException {
...
public List<StuduserDTO> makelist(ResultSet rs) throws SQLException {
List<StuduserDTO> list=new ArrayList<StuduserDTO>();
while (rs.next()) {
